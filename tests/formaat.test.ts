...
test("een weekplan van een nieuwere Cupboard wordt gelezen als onbekend en niet geschreven", async () => {
	...
	assert.match(plan.unreadable ?? "", /newer Cupboard/);
...
test("een lijst van een nieuwere Cupboard staat op slot", async () => {
	const h = makeHarness(loadFixture("week-basis"), {
	...
	assert.match(known.frozen ?? "", /newer Cupboard/);
	assert.match(h.plugin.lists.frozenReason() ?? "", /update Cupboard on this device/);
	...
	await assert.rejects(h.plugin.lists.markBought(known, ui), /newer Cupboard/);
